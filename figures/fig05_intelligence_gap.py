...
from src.paths import resolve_geotiff

PROBS_PATH = COLAB_RESULTS_V2 / "probs_full.npz"
if not PROBS_PATH.exists():
    PROBS_PATH = COLAB_RESULTS_LOCAL / "probs_full.npz"

GEOTIFF_ARMORED = resolve_geotiff("blindada")
...
def build():
    ...
    legend_bp = [
        Patch(facecolor=PALETTE["muted"], alpha=0.85, label="Outside Intel. Gap"),
        Patch(facecolor=PALETTE["highlight"], alpha=0.85, label="Inside Intel. Gap"),
    ]
    ...
    axC.set_title(f"C) Intelligence Gap (P{INTGAP_PERCENTILE}) \u2014\n"
                  f"{igap_mask.mean()*100:.1f}% of terrain",
                  fontsize=8.8, fontweight="bold", color=PALETTE["text"],
                  loc="left", pad=4)
    ...
    fig.legend(handles=cls_handles, loc="outside lower center", ncol=3,
               fontsize=7.2, frameon=False)

    fig.suptitle(
        f"Intelligence Gap identification \u2014 {LABEL_FRAC} fraction",
