...
def build():
    set_mpl_style()
    fig, ax = plt.subplots(figsize=(FIG_WIDTH_IN, FIG_WIDTH_IN * 0.62))
    ax.set_xlim(0, 16)
    ax.set_ylim(0, 10)
    ax.set_aspect("equal")
    ax.axis("off")
    ...
    for y, h, cor, rot in bands:
        ...
        ax.text(0.15, y + h - 0.10,
                rot, ha="left", va="top", fontsize=7.0,
    ...
    boxes_2 = [
        (1.20, "Rasterization", "100 m grid \u00b7 3598\u00d73354"),
        (5.20, "12.07 M nodes", "17 features / node"),
        (8.90, "k-NN graph (k=8)", "edges: dist. + elev. diff."),
        (12.60, "Spatial split", "blocks + 500 m buffer"),
    ]
    wg, hg = 3.10, 1.20
    y2 = 5.95
    centers_2 = []
    ...
    _box(ax, 12.10, 3.55, 3.30, 1.10,
         ...
         title_color=PALETTE["highlight"], lw=1.0)
    ax.text(8, 3.30,
            "output: Go \u00b7 Slow Go \u00b7 No Go  per fraction   \u00b7   "
            "52,492 params \u00b7 horizon 100 \u2192 200 \u2192 300 m",
            ha="center", va="top", fontsize=6.6,
            color=PALETTE["muted"], style="italic")

    _box(ax, 4.50, 0.75, 7.00, 1.05,
         "Multi-band GeoTIFF \u00b7 EPSG:4326",
         "4 bands (one per fraction) \u00b7 interoperable with ArcGIS Pro / ENVI",
    ...
    _arrow(ax, 11.72, 4.10, 12.08, 4.10,
           color=PALETTE["highlight"], lw=0.9)
    _arrow(ax, 12.10 + 3.30 / 2, 3.53, 4.50 + 7.00 / 2, 1.82,
