...
def build():
    ...
    head_w = 2.40
    head_x0 = 0.15
    heads = ["Dismounted", "Motorized", "Mechanized", "Armored"]
    head_gap = 0.12
    ...
    ax.text(lx + lw_box / 2, ly + lh - 0.30,
            "COO-Informed Loss (CIL) \u2014 physics-informed",
            ha="center", va="center", fontsize=7.0, fontweight="bold",
            color=PALETTE["highlight"])

    eq = (r"$\mathcal{L} = \sum_{f} w_f\, \mathcal{L}_{CE}^{f} + "
          r"\lambda_1 \mathcal{L}_{slope} + \lambda_2 \mathcal{L}_{NDVI} + "
          r"\lambda_3 \mathcal{L}_{water}$")
    ax.text(lx + lw_box / 2, ly + lh - 0.95, eq,
            ha="center", va="center", fontsize=8.0,
            color=PALETTE["text"])

    items = [
        ("CE",        "cross-entropy with class weights [1; 3\u20135; 1]"),
        (r"$\mathcal{L}_{slope}$", r"slope above NATO threshold ($\lambda_1=0.5$)"),
        (r"$\mathcal{L}_{NDVI}$", r"dense vegetation (high NDVI) ($\lambda_2=0.5$)"),
        (r"$\mathcal{L}_{water}$", r"water body overlap (NDWI mask) ($\lambda_3=1.0$)"),
    ]
    y_i = ly + lh - 1.45
